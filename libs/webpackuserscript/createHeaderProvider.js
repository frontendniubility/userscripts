...
module.exports = function createHeaderProvider(packageJson, headerFnOrObj) {
    ...
    const pkgSupportUrl = typeof packageJson.bugs === 'string' ? packageJson.bugs : typeof packageJson.bugs === 'object' && typeof packageJson.bugs.url === 'string' ? packageJson.bugs.url : '';

    // return user defined headers
    let _headerFn = typeof headerFnOrObj === 'function' ? headerFnOrObj : () => headerFnOrObj || {};
...
function trimFalsyLeaves(obj) {
    ...
    if (typeof obj === 'object') {
        const filtered = Object.entries(obj).filter(([, value]) => Boolean(trimFalsyLeaves(value)));
        return filtered.length > 0
            ? filtered.reduce((newObj, [key, value]) => {
                  newObj[key] = value;
                  return newObj;
              }, {})
            : undefined;
