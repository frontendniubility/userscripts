...
module.exports = class webpackTestPlugin {
	...
	apply(compiler) {
		Object.keys(compiler.hooks).forEach(function (crKey) {
			...
			// 
			webpackTestPlugin.log("crtap", crKey, "", ` ${compilerHook._args != null ? "args:  " + compilerHook._args.join(", ") : "null"}`)
							...
							if (!compilationHook.for) {
								try {
									if (cnKey != "log") {
										...
										webpackTestPlugin.log("cntap", crKey, cnKey, ` ${compilerHook._args != null ? "args:  " + compilerHook._args.join(", ") : "null"}`)
