...
export function getTeacherInfoFromDetailPage(tinfo_saved = {}, jqr, tinfo_latest = {}) {
  ...
  let tinfo = {
    ...
    teacherStar: Number(
      jqr
        .find('.s-t-top>.s-t-top-details.f-cb>.s-t-top-left>.teacher-left-right>.teacher-star')
        .text()
    ),
    certificaties: jqr
      .find(
        '.s-t-top>.s-t-top-details.f-cb>.s-t-top-left>.teacher-left-right>.teacher-icon-tag>span:eq(0)'
      )
      .text(),
    suitables: jqr
      .find(
        '.s-t-top>.s-t-top-details.f-cb>.s-t-top-left>.teacher-left-right>.suitable>span:not(:first)'
      )
      .map(function(i, v) {
        return $(v).text()
      })
