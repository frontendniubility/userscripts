...
function executeFilters(uifilters) {
  ...
  $.each($('.item'), function(i, item) {
    ...
    if (!isNaN(tinfo.thumbupRate))
      ret = tinfo.thumbupRate >= uifilters.rate1 && tinfo.thumbupRate <= uifilters.rate2
    ...
    if (!isNaN(tinfo.favoritesCount))
      tinfo.favoritesCount >= uifilters.fc1 && tinfo.favoritesCount <= uifilters.fc2 && ret
    if (ret) {
      if (node.is(':hidden')) {
...
剩余${sessionStorage.getItem('selectedTimeSlotsRemain')}/${sessionStorage.getItem(
        'selectedTimeSlotsTotal'
      )}个时段，<br><br>
当前时段约${totalPages * 28}个教师，获取第${curPageId}/${totalPages}页，进度${Math.floor(
        (curPageId / totalPages) * 100
      )}%,<br>
      ...
      Pace.track(function() {
        ...
        $.ajax({
          url:
            ...
            '//www.51talk.com/TeacherNew/teacherComment?tid=' +
            tid +
            '&type=bad&has_msg=1',
          type: 'GET',
          dateType: 'html',
          ...
          error: function(data) {
            console.log(
              'xhr error when getting teacher ' +
                JSON.stringify(jqel) +
                ',error msg:' +
                JSON.stringify(data)
            )
